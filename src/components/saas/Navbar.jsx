...
import { FaCoins, FaUser, FaSignOutAlt, FaChevronDown, FaRocket, FaBars, FaTimes, FaGithub } from "react-icons/fa";
...
export function Navbar() {
  ...
  const navLinks = [
    { name: "AICLIP Studio", href: "/" },
    { name: "My Clips", href: "/creations" },
  ...
  return (
    <nav className="h-20 border-b border-glass-border bg-glass-bg backdrop-blur-3xl sticky top-0 z-[100] px-4 md:px-12 flex items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        ...
        <a 
          href="https://vercel.com/new/clone?repository-url=https://github.com/SamurAIGPT/ai-clipping-generator"
          target="_blank"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-20 left-0 right-0 bg-[var(--solid-bg)]/90 backdrop-blur-2xl border-b border-glass-border shadow-2xl flex flex-col md:hidden z-[110] p-4 gap-2 isolate"
          >
            {navLinks.map((link) => {
              if (!session && link.href === "/creations") return null;
              const isActive = pathname === link.href;
